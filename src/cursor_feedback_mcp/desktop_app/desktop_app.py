...
# 導入現有的 Cursor Feedback MCP 模組
try:
    from cursor_feedback_mcp.debug import server_debug_log as debug_log
    from cursor_feedback_mcp.web.main import WebUIManager, get_web_ui_manager
except ImportError as e:
    # 在這裡無法使用 debug_log，因為導入失敗
    sys.stderr.write(f"無法導入 Cursor Feedback MCP 模組: {e}\n")
    sys.exit(1)
...
class DesktopApp:
    ...
    async def launch_tauri_app(self, server_url: str):
        ...
        try:
            from cursor_feedback_mcp.desktop_release import __file__ as desktop_init
            ...
            # 定義平台到二進制文件的映射
            if system == "windows":
                tauri_exe = desktop_dir / "cursor-feedback-mcp-desktop.exe"
            elif system == "darwin":  # macOS
                # 檢測 Apple Silicon 或 Intel
                if machine in ["arm64", "aarch64"]:
                    tauri_exe = (
                        desktop_dir / "cursor-feedback-mcp-desktop-macos-arm64"
                    )
                else:
                    tauri_exe = (
                        desktop_dir / "cursor-feedback-mcp-desktop-macos-intel"
                    )
            elif system == "linux":
                tauri_exe = desktop_dir / "cursor-feedback-mcp-desktop-linux"
            else:
                # 回退到通用名稱
                tauri_exe = desktop_dir / "cursor-feedback-mcp-desktop"
            ...
            if tauri_exe.exists():
                ...
                # 嘗試回退選項
                fallback_files = [
                    desktop_dir / "cursor-feedback-mcp-desktop.exe",
                    desktop_dir / "cursor-feedback-mcp-desktop-macos-intel",
                    desktop_dir / "cursor-feedback-mcp-desktop-macos-arm64",
                    desktop_dir / "cursor-feedback-mcp-desktop-linux",
                    desktop_dir / "cursor-feedback-mcp-desktop",
                ]
            ...
            tauri_exe = (
                ...
                / "target"
                / "debug"
                / "cursor-feedback-mcp-desktop.exe"
            )
