...
class PortManager:
    ...
    def kill_process_on_port(port: int, force: bool = False) -> bool:
        ...
        try:
            ...
            # 檢查是否是自己的進程（避免誤殺）
            if "cursor-feedback-mcp" in process_info["cmdline"].lower():
                debug_log("檢測到 Cursor Feedback MCP 相關進程，嘗試優雅終止")
    ...
    def _should_cleanup_process(process_info: dict[str, Any]) -> bool:
        ...
        Returns:
            bool: 是否應該清理該進程
        """
        # 檢查是否是 Cursor Feedback MCP 相關進程
        ...
        if any(
            keyword in cmdline
            for keyword in ["cursor-feedback-mcp", "cursor_feedback_mcp"]
        ):
            return True
